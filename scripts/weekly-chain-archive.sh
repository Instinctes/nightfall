...
# And on every real run, before anything is uploaded or deleted.
#
# This used to be a CI step, which stopped being a place it could run: this
# script is workspace-only and the mirror's CI cannot see it. A check that
# guards a deletion belongs next to the deletion anyway — it costs nothing and
# needs no network, so there is no reason for it to live anywhere else.
self_test || exit 1
...
if [ "$DEPLOY" = "1" ]; then
    # One gate, not two.
    #
    # This block used to run three of the site checks by hand and check the
    # wrangler login itself. deploy-website.sh runs all nine plus the two
    # guards that matter most here — it refuses to publish the mirror, and it
    # refuses a tree that is missing a page the site is currently serving.
    # Two lists of "what must be true before the site goes live" drift; the
    # one that is not looked at drifts silently, and the checks it forgets are
    # the ones nobody misses until a user reports the bug.
    command -v node >/dev/null || die "node is not installed"
    "$ROOT/scripts/deploy-website.sh" --check \
        || die "the website checks failed — nothing has been built or published."
    step "website...... checks pass"
fi
...
if [ "$DEPLOY" = "1" ]; then
    # Through the same gate as a plain site deploy. It re-runs the checks
    # against the manifest just written above — which is the point, since that
    # file did not exist when the preflight ran — then publishes and reads the
    # home page and every live page back.
    "$ROOT/scripts/deploy-website.sh" \
        || die "the website deploy failed. The archive is on the release; the
site still points at the previous one, which is behind but real."
fi
...
if [ "$DEPLOY" = "1" ]; then
    # The home page and the other pages were already read back by
    # deploy-website.sh. What is left is the part only this script can check,
    # because only this script knows which height it just published: that the
    # manifest the page reads is the new one, and that the archive it names can
    # actually be fetched.
    say "==> checking the archive on the live site"
    step "waiting 60 seconds for the deployment to propagate"
    sleep 60
