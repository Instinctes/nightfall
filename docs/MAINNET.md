...
4. Same software version (protocol v4 / wire v1)
...
| Block time target | ~15s (mainnet pacing) |
| Initial difficulty | 5,000,000 (LWMA retarget every block) |
...
- **Amounts are confidential** (Pedersen + Bulletproofs) and **no recipient
  appears on chain** (one-time output keys). The **transaction graph is still
  linkable** — Mimblewimble inputs reference specific commitments, and kernel
  aggregation / cut-through are not implemented.
- **No network-layer privacy.** No Dandelion++; the first relaying node is
  probably the origin.
- **PoW is Blake3**, not the promised memory-hard Argon2id. ASIC-friendly.
...
## 8. Devnet dry-run (before mainnet)
