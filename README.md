...
[![Protocol](https://img.shields.io/badge/protocol-v7%20Nightproof-8b5cf6)](docs/SPEC.md)
...
> [!NOTE]
> **v0.6.3 — catch up from a fork.** Same chain and wire as 0.6.1/0.6.2.
> 0.6.2 kept the socket open but could not leave a one-block fork: it asked
> for a page starting at a height it already had, threw the rest away, and
> never weighed the heavier chain. 0.6.3 does. Details in
> [v0.6.3](https://github.com/Instinctes/nightfall/releases/tag/v0.6.3).

> [!IMPORTANT]
> **Running 0.5.4 or earlier? Update.** From v0.6.1 the network speaks wire
> protocol v5, and older builds cannot complete a handshake — they will not
> sync, will not propagate, and anything they mine lands on a branch no other
> node accepts.
>
> This is not a chain reset. Genesis is unchanged (`c8614333…`), protocol
> stays v7, and **every coin you have already mined is untouched and still
> yours** — an old wallet gets its balance back the moment it updates.
>
> The reason is 0.5.3 and earlier verify a chain reorganisation while holding
> the node's global lock, which freezes them for tens of seconds at a time and
> makes them mine on a tip the network has already left. That does not just
> hurt the node running it; every other node has to reconcile the branches it
> produces. Details in
> [v0.5.4](https://github.com/Instinctes/nightfall/releases/tag/v0.5.4) and
> [v0.6.1](https://github.com/Instinctes/nightfall/releases/tag/v0.6.1).
...
```bash
shasum -a 256 -c SHA256SUMS-0.6.3.txt
```
</details>
...
```powershell
certutil -hashfile nightfall-core-0.6.3-windows-x64.exe SHA256
```
</details>
...
- **No network-layer privacy.** No Dandelion++. The node that first relays your transaction is probably its origin.
