...
**Current mainnet wallet release: [1.0.3](https://github.com/Instinctes/nightfall/releases/tag/v1.0.3).**
Core desktop and the responsive [web wallet](https://nightfallcoin.org/wallet/)
share the new violet design. Mainnet mining, sending and receiving remain available.
**Atomic swap has been withdrawn** and is not part of the wallet — see
[docs/SWAP-WITHDRAWN.md](docs/SWAP-WITHDRAWN.md). No consensus rule changed;
node operators and miners need do nothing.
...
```bash
shasum -a 256 -c SHA256SUMS-1.0.3.txt
```
</details>
...
```powershell
certutil -hashfile nightfall-core-1.0.3-windows-x64.exe SHA256
```
</details>
...
## NIGHT ↔ BTC atomic swaps — withdrawn

Direct NIGHT ↔ BTC atomic swap was built, reviewed, and **withdrawn before
1.0.3**. It is not in the wallet and is not scheduled for a later release.

The reason is not a bug. Mimblewimble has no script language, so a NIGHT lock
cannot refund itself on a timer the way a Bitcoin lock can. A user could end up
with NIGHT locked permanently, or — if a redeem was published late and did not
confirm — could lose both sides. No amount of implementation work removes that;
it is a property of the construction. Shipping it without an independent audit,
to a handful of users willing to run a full `bitcoind`, was not a trade worth
making for a first mainnet release.

[docs/SWAP-WITHDRAWN.md](docs/SWAP-WITHDRAWN.md) is the full record: what could
not be fixed, what the last review found, what was removed, and the one piece of
storage compatibility that was deliberately kept.

**No consensus rule, protocol version, genesis parameter or emission schedule
changed.** There is no fork and no migration.
...
| [docs/SWAP-WITHDRAWN.md](docs/SWAP-WITHDRAWN.md) | Why atomic swap was withdrawn before 1.0.3 |
