...
> [!NOTE]
> **v0.6.2 — stay on the tip.** Same chain, same genesis, same wire as 0.6.1.
> Wallets behind NAT no longer lag a few blocks and orphan their own mining
> rewards: connections stay open, and a new block is written onto that socket.
> 0.6.1 still peers. Details in
> [v0.6.2](https://github.com/Instinctes/nightfall/releases/tag/v0.6.2).
...
```bash
shasum -a 256 -c SHA256SUMS-0.6.2.txt
```
</details>
...
```powershell
certutil -hashfile nightfall-core-0.6.2-windows-x64.exe SHA256
```
</details>
