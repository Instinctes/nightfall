...
**Live:** https://nightfallcoin.husref-huremovic.workers.dev
...
```bash
U=https://nightfallcoin.husref-huremovic.workers.dev
...
Once a domain is on the Cloudflare account:

```bash
wrangler deploy --route "nightfallcoin.org/*"
```

or add it under Workers & Pages → nightfallcoin → Settings → Domains & Routes.
...
- The **seed node** address, once one exists, belongs both here and in
  `NetworkId::seed_nodes()` in `crates/nightfall-types/src/lib.rs`.
